...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={clsx("font_metric_caption uppercase px-1.5 py-0.5 rounded-sm border",
    ...
    <div className="mt-3 space-y-2" aria-label="Escalation timeline">
      ...
      {events.map((event) => {
        ...
        return (
          <div
            key={event.id}
            className={clsx("flex items-center gap-2 rounded-lg border px-3 py-2",
              tier.bg,
              tier.border
            )}
          >
            <TierIcon
              ...
              aria-hidden="true"
            />
            <span className={clsx("font_metric_caption", tier.color)}>
              {tier.label}
            </span>

...
function getEventColor(event: InsulinTimelineEvent, palette: ChartPalette): string {
  switch (event.kind) {
    case "basal":
...
function drawInsulinBaseline(chart: uPlot, palette: ChartPalette): void {
  ...
  chart.ctx.lineWidth = typeof window === "undefined" ? 1 : window.devicePixelRatio || 1;
  chart.ctx.beginPath();
...
function drawMarker(
  ...
  if (event.kind === "basal") {
    ...
    chart.ctx.fillRect(x - barWidth / 2, y, barWidth, Math.max(pixelRatio, baseline - y));
    chart.ctx.globalAlpha = 1;
    chart.ctx.strokeRect(x - barWidth / 2, y, barWidth, Math.max(pixelRatio, baseline - y));
    chart.ctx.restore();
    return;
  }
...
function resolveYDomain(data: InsulinTimelineEvent[]): [number, number] {
  const maxBolus = data.reduce(
    (current, event) => event.kind === "bolus" ? Math.max(current, event.units) : current,
    0
  );
...
export function InsulinTimeline({
  ...
  const visibleData = useMemo(
    () => data.filter((event) => event.timestamp >= xDomain[0] && event.timestamp <= xDomain[1]),
    [data, xDomain]
  );
  ...
  useEffect(() => {
    ...
    const chartData = visibleData.length > 0
      ? visibleData
      : [
      ...
      hooks: {
        ...
        setCursor: [
          (chart) => {
            ...
            const event = typeof index === "number" ? dataRef.current[index] : null;
            const plotWidth = Math.max(1, chart.bbox.width / (window.devicePixelRatio || 1));
            const hoverWindowMs = (
              (xDomain[1] - xDomain[0]) * INSULIN_HOVER_PROXIMITY_PX
            ) / plotWidth;
            const nearbyEvent = event && Math.abs(event.timestamp - timestamp) <= hoverWindowMs
              ? event
              : null;
  ...
  const accessibleSummary = visibleData.length === 0
  ...
  return (
    <section className="border-t border-border-default" aria-label="Insulin doses">
      <div
      ...
      >
        ...
        {isLoading ? (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-surface-primary/80 text-center">
            <p className="font_body_3 text-signal-error-text">Unable to load insulin doses</p>
            <button
              type="button"
              onClick={onRetry}

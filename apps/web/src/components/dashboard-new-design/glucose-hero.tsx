...
export interface GlucoseHeroProps {
  ...
  isLoading?: boolean;
  /** Removes the card shell when the hero is placed inside another container. */
  embedded?: boolean;
  /** Shows the pump metric block inside the hero. */
...
function LoopStatusBadge({
  ...
  return (
    <div
      className={clsx("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full","font_metric_caption border",
        style.pill
      )}
      role="status"
    ...
    >
      ...
      <span
        className={clsx(secondaryTextClassName, "font_body_3 border-l border-border-hover pl-1.5 ml-0.5")}
        aria-hidden="true"
      >
        {sourceName}
      </span>
...
export function GlucoseHero({
  ...
  const metricItemClassName = clsx(
    "flex flex-col items-center",
    embedded &&"lg:items-start"
  );
  const metricSeparatorClassName = clsx(
    "hidden sm:block w-px h-6 bg-surface-tertiary",
    embedded &&"lg:hidden"
  );
  return (
      ...
      >
        ...
        {showPumpStats && (
          ...
          >
            ...
            >
              <span className={clsx(secondaryTextClassName, "font_metric_caption uppercase")} aria-hidden="true">
                IoB
              </span>
            ...
            {safeCob !== null && (
              <>
                ...
                >
                  <span
                    className={clsx(secondaryTextClassName, "font_metric_caption uppercase")}
                    aria-hidden="true"
                  >
                    COB
                  </span>
